...
ID3D11ShaderResourceView* TerrainClass::GetNormalTexture()
{
...
bool TerrainClass::LoadHeightMap(char* filename)
{
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].y = (float)height * 3;
...
bool TerrainClass::Smooth(ID3D11Device* device, bool keydown)
{
	int counter = 1;
	int index;
	bool result;
	if (keydown)
	{
		for (int j = 0; j < m_terrainHeight; j++)
		{
			for (int i = 0; i < m_terrainWidth; i++)
			{
				...
				if (index - 257 >= 0)
				{
					m_heightMap[index].y += m_heightMap[index - 257].y;
					counter++;
				}

				if (index - 256 >= 0)
				{
					m_heightMap[index].y += m_heightMap[index - 256].y;
					counter++;
				}

				if (index - 255 >= 0)
				{
					m_heightMap[index].y += m_heightMap[index - 255].y;
					counter++;
				}
				...
				if (index + 255 <= ((m_terrainHeight*m_terrainWidth) - 1))
				{
					m_heightMap[index].y += m_heightMap[index + 255].y;
					counter++;
				}

				if (index + 256 <= ((m_terrainHeight*m_terrainWidth) - 1))
				{
					m_heightMap[index].y += m_heightMap[index + 256].y;
					counter++;
				}

				if (index + 257 <= ((m_terrainHeight*m_terrainWidth) - 1))
				{
					m_heightMap[index].y += m_heightMap[index + 257].y;
					counter++;
				}
...
bool TerrainClass::AddNoise(ID3D11Device* device, bool keydown)
{
	bool result;
	int index;
	perlinnoise pn;
	...
	if (keydown)
	{
		for (int j = 0; j < m_terrainHeight; j++)
		{
			for (int i = 0; i < m_terrainWidth; i++)
			{
				index = (m_terrainHeight * j) + i;
				m_heightMap[index].x = (float)i;
				m_heightMap[index].y += (float)(pn.octavePerlin(i * 0.2, 0.0, j * 0.2, 8, 0.7, pn)) * 2;
				m_heightMap[index].z = (float)j;
			}
		}
...
bool TerrainClass::FaultTerrain(ID3D11Device* device, bool keydown, bool circleFault)
{
	...
	if (keydown)
	{
		for (int t = 0; t < 50; t++) {
			if (!circleFault) { // line fault
				...
				float a = sin(v);
				float b = cos(v);
				float d = sqrt(m_terrainHeight*m_terrainHeight + m_terrainWidth*m_terrainWidth);
				...
				int randX = rand() % (m_terrainHeight + 1);
				int randZ = rand() % (m_terrainWidth + 1);
				int randCircSize = rand() % ((m_terrainHeight + m_terrainWidth) / 10); // circle diameter
				...
				for (int z = 0; z < m_terrainWidth; z++) {
					for (int x = 0; x < m_terrainHeight; x++) {
						...
						if (fabs(pd) <= 1.0f) { // if the vertex is within the circle, displace it upwards
							float diff = (displacement / 2.0f + cos(pd*3.14f)*displacement / 2.0f);
							m_heightMap[index].y += diff * 7; // constrain to maxHeight
